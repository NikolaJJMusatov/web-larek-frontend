...
### Класс ProductItem наследуется от Model
(создает данные о продукте с целью применения в слое отображения или бизнес-логики)
...
- preview (ID товара для предпросмотра)
...
- setPreview (предпросмотр продукта и событие об изменении предпросмотра)
...
- setButton (управляет кнопкой оформления заказа в корзине)
...
```
методы:
- toggleButtons (управляет кнопками способа оплаты)
...
preview:changed (изменение окна предпросмотра)
